YMLParser.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

YMLParser.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
YMLParser.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    for (var i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="ymlparser-btn ymlparser-btn-default {1}" action="{2}" title="{3}"></button></li>',
